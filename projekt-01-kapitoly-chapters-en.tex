...
\chapter{Conclusion}
The initial part of this work was dedicated to the topic of testing the GUIs. Several widely used approaches were discussed and concluded by the Model-based testing, which will be used for development of this work.

The architecture of the assistive technologies in GNOME environment are discussed as well. Several tools used for exploration, verification and extraction of accessibility metadata were described, including Python libraries dogtail and pyatspi. Metadata used by assistive technologies are providing complex information about running applications and they are already being used in various testing frameworks. Limitation of accessibility might be partially solved by the computer vision algorithms, mainly OCR. These tools also provide additional level of verification of content displayed during testing.

Final chapter is dedicated to the proposed solution. A tool written in Python3, with ability to extract an abstract model application from metadata provided by accessibility in GNOME environment. The tool afterwards converts the model to the Event-flow graph and generates test cases suitable for automated execution. The chapter also revealed test case management with several challenges that need to be addressed to provide reliable results.  