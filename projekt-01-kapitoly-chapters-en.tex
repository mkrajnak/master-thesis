...
Result of the test generation process will be a set of \verb|.feature| files describing test cases in language Gherkin. The test cases will be executable in Python's test framework behave. 

behave is one of the most popular BDD frameworks in Python. It's similar to other Gherkin-based frameworks despite not holding the official Cucumber designation. It has two primary layers:
\begin{enumerate}
...
A test case written in behave is intuitive and easy to understand, which is important for quick re-evaluation of failed test cases.  
...
\begin{lstlisting}[language=Gherkin,,xleftmargin=3.5ex,caption={.feature file example with a test case}]
